...
A high-performance, serverless Content Delivery Network (CDN) edge proxy for `nanoo.cloud` infrastructure. Built on top of **Cloudflare Workers** to proxy requests securely to private **Backblaze B2** buckets.
...
The Cloudflare Worker intercepts requests to `cdn.nanoo.cloud`, signs them on-the-fly using AWS Signature Version 4 (`aws4fetch`), and fetches the objects from the B2 S3 endpoint.
